...
app.post("/protect", upload.single('data'), function (request, response) {
    if (request.file) {
        var inputFileName = request.file.filename;
        var inputFilePath = `${__dirname}/temp/${inputFileName}}`; // путь принятого файла
        ...
        if (ext === "docx" /*|| ext === "doc"*/) { // триал версия работает только с файлами .docx
            ...
            var outputFilePath = `${__dirname}/temp/upd_${inputFileName}`; // путь нового документа
...
app.post("/digitallySigne", upload.single('data'), function (request, response) {
    if (request.file) {
        var inputFileName = request.file.filename;
        var inputFilePath = `${__dirname}/temp/${inputFileName}`; // путь принятого файла
        var ext = inputFileName.split(".").pop(); // расширение принятого файла
        ...
        if (ext === "docx" /*|| ext === "doc"*/) { // триал версия работает только с файлами .docx
            ...
            var xmlDocxTools = new XmlDocxTools(tempPath);
            //  var config = xmlDocxTools.getProtectedConfigXml(inputFilePath,outputFilePath); // защита от редактирования
            var config = xmlDocxTools.getDigitalConfigXml(inputFilePath, outputFilePath, privaKeyPath, certificatePath); // установка цифровой подписи
            var content = xmlDocxTools.getContentXml();
            var settings = xmlDocxTools.getSettingsXml();
            ...
            // асинхронная проверка файла
            //checkFile(outputFilePath, delay, function (err, data) {
            checkFile(inputFilePath, 1000, function (err, data) {
                if (err) {
                    response.sendStatus(500);
                    logger.error("error in process generating document: " + outputFilePath);
                } else {
...
app.post("/test", upload.single('data'), function (request, response) {
    if (request.file) {
        var inputFileName = request.file.filename;
        var inputFilePath = `${__dirname}/temp/${inputFileName}`; // путь принятого файла
        var ext = inputFileName.split(".").pop(); // расширение принятого файла
        ...
        if (ext === "docx" /*|| ext === "doc"*/) { // триал версия работает только с файлами .docx
            ...
            var xmlDocxTools = new XmlDocxTools(tempPath);
            //  var config = xmlDocxTools.getProtectedConfigXml(inputFilePath,outputFilePath); // защита от редактирования
            var config = xmlDocxTools.getConfigXml(inputFilePath, outputFilePath); // установка цифровой подписи
            var content = xmlDocxTools.getContentXml();
            var settings = xmlDocxTools.getSettingsXml();
